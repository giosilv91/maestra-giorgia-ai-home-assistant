...
VISION_DIR=DATA/'vision'; VISION_DIR.mkdir(parents=True,exist_ok=True)
...
class Store:
    def __init__(self):
        ...
        with self.lock:
            ...
            CREATE TABLE IF NOT EXISTS book_scans(id INTEGER PRIMARY KEY AUTOINCREMENT,student_id INTEGER,title TEXT,author TEXT,topic TEXT,grade_level TEXT,summary_short TEXT,summary_simple TEXT,keywords TEXT,questions TEXT,map_text TEXT,image_path TEXT,created_at TEXT);
            CREATE TABLE IF NOT EXISTS homework_reviews(id INTEGER PRIMARY KEY AUTOINCREMENT,student_id INTEGER,subject TEXT,task_type TEXT,image_path TEXT,detected_text TEXT,error_json TEXT,correction_text TEXT,child_explanation TEXT,recovery_activity TEXT,created_at TEXT);
            CREATE TABLE IF NOT EXISTS handwriting_reviews(id INTEGER PRIMARY KEY AUTOINCREMENT,student_id INTEGER,writing_type TEXT,image_path TEXT,legibility TEXT,spacing_notes TEXT,alignment_notes TEXT,shape_notes TEXT,motor_notes TEXT,strengths TEXT,improvements TEXT,recommended_exercises TEXT,created_at TEXT);
            """)
            self.db.commit()
...
def _gemini_multimodal(prompt, images):
    settings=load_settings(); key=settings.get('gemini_api_key','').strip()
    if not key: raise RuntimeError('Per analizzare le foto serve la Gemini API key')
    model=settings.get('gemini_model') or 'gemini-3.8-flash'
    parts=[{'text':PROMPT+'\n\n'+prompt}]
    for img in images[:6]:
        data=(img.get('data') or '').strip()
        if not data: continue
        parts.append({'inlineData':{'mimeType':img.get('mime') or 'image/jpeg','data':data}})
    if len(parts)==1: raise RuntimeError('Nessuna immagine ricevuta')
    url=f'https://generativelanguage.googleapis.com/v1beta/models/{model}:generateContent?key={key}'
    ...
    for attempt in range(3):
        try:
            ...
            text=''.join(p.get('text','') for p in out.get('candidates',[{}])[0].get('content',{}).get('parts',[])).strip()
            if text:return text
            raise RuntimeError(out.get('error',{}).get('message','Risposta visiva Gemini non valida'))
        except urllib.error.HTTPError as e:
            detail=e.read().decode(errors='ignore'); last=RuntimeError(detail or str(e))
            if e.code not in (429,500,502,503,504) or attempt==2: break
            time.sleep(1.5*(attempt+1))
        except Exception as e:
            last=e
            if attempt==2: break
            time.sleep(1.0*(attempt+1))
    raise RuntimeError(f'Visione Gemini non disponibile: {last}')
...
,'',raw)
    a=raw.find('{'); z=raw.rfind('}')
    if a<0 or z<a: raise RuntimeError('Risposta AI non strutturata')
    return json.loads(raw[a:z+1])

def _save_jpeg(b64data,prefix):
    if not b64data:return ''
    raw=base64.b64decode(b64data)
    if len(raw)>8*1024*1024: raise RuntimeError('Immagine troppo grande')
    name=f"{prefix}_{int(time.time()*1000)}.jpg"
    path=VISION_DIR/name; path.write_bytes(raw); return str(path)

def vision_analyze(mode,images,student_context='',subject='',task_type='',writing_type=''):
    if mode=='book':
        prompt="""Analizza le foto di copertina/pagine come assistente didattica primaria. Restituisci SOLO JSON:
{"title":"","author":"","topic":"","grade_level":"","summary_short":"","summary_simple":"","keywords":[""],"questions":[""],"map":{"center":"","branches":[{"label":"","items":[""]}]}}
Regole: se titolo/autore non sono leggibili scrivi "non leggibile"; riassunto breve massimo 8 righe; versione semplice con frasi corte; non inventare contenuti non visibili; se sono fotografate poche pagine, riassumi SOLO cio che si vede."""
    elif mode=='homework':
        prompt=f"""Analizza il compito fotografato. Materia indicata: {subject or 'non specificata'}. Tipo: {task_type or 'compito'}.
Restituisci SOLO JSON:
{{"subject":"","detected_text":"","errors":[{{"label":"","correction":"","note":"","x":0,"y":0,"w":0,"h":0}}],"strengths":[""],"child_explanation":"","recovery_activity":""}}
Coordinate x,y,w,h sono NORMALIZZATE 0-1000 rispetto all'immagine. Inserisci un box solo se la zona dell'errore e chiaramente individuabile. Controlla ortografia, calcolo, procedimento e consegna in base a cio che e realmente visibile. Non inventare errori. Spiegazione al bambino molto breve e rispettosa."""
    elif mode=='handwriting':
        prompt=f"""Osserva questa scrittura in ottica DIDATTICA e GRAFOMOTORIA. Tipo indicato: {writing_type or 'non specificato'}.
Non inferire personalita, ansia, intelligenza, diagnosi, DSA o condizioni cliniche dalla grafia.
Restituisci SOLO JSON:
{{"writing_type":"","legibility":"","spacing":"","alignment":"","letter_shape":"","motor_observations":"","strengths":[""],"improvements":[""],"exercises":[""]}}
Descrivi solo caratteristiche visibili: leggibilita, dimensione, spaziatura, rigo, regolarita, forma, continuita/controllo apparente del tratto. Usa formule come "si osserva" e consigli pratici. Massimo 5 esercizi."""
    else: raise RuntimeError('Modalita visione non valida')
    text=_gemini_multimodal(prompt,images)
